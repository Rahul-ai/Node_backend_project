...
class GetConfigError extends Error {
  constructor(message, _error) {
    super((0, import_errorHelpers.addVersionDetailsToErrorMessage)(`${import_chalk.default.redBright.bold("Get config: ")}${message}`));
    this._error = _error;
  }
}
...
async function getConfigNodeAPI(options) {
  ...
  const error = (0, import_ts_pattern.match)(configEither.left).with({ type: "node-api" }, (e) => {
    ...
    const actualError = (0, import_function.pipe)(E.tryCatch(() => JSON.parse(errorOutput), () => {
      ...
      const message = (0, import_ts_pattern.match)(errorOutputAsJSON).with({ error_code: "P1012" }, (error2) => {
        return import_chalk.default.redBright(`Schema Parsing ${error2.error_code}

`) + error2.message + "\n";
      }).otherwise((error2) => {
        return import_chalk.default.redBright(`${error2.error_code}
...
`) + defaultMessage;
      }).with({ error_code: import_ts_pattern.P.string }, (error2) => {
        return import_chalk.default.redBright(`${error2.error_code}
